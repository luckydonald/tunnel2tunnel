...
- **Building on a <200MB RAM VPS**: `cargo build --release` and `npm run build` both need far more than 200MB peak RSS — a swapfile on the host is mandatory, the build changes in the Dockerfile only reduce how much of it gets used. **Nothing in this repo provisions that swapfile** — it's asserted as an external host prerequisite, not automated (docker-compose can't reach outside the container to do it). Before relying on any in-container memory tuning, verify swap actually exists on the real deploy host: `swapon --show`; if empty, create one (e.g. `fallocate -l 2G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile`, plus an `/etc/fstab` entry to persist it) sized to comfortably cover peak build RSS. Don't add `sentry`'s `rustls` feature to save build memory — it looks lighter (drops `native-tls`/`openssl-sys`) but reqwest's `rustls` feature hard-codes the `aws-lc-rs` crypto provider, which compiles a vendored C library via cmake; that's heavier than `native-tls`, which just links against Debian's already-built `libssl` via `pkg-config` (no compilation). Keep `native-tls`. `CARGO_BUILD_JOBS` and the Vue build's Node heap ceiling (`NODE_MAX_OLD_SPACE_MB`) are exposed as Docker build args (defaults `1` / `2048`, see `docker-compose.yml`/`docker-compose.coolify.yml`) so they can be tuned per-host without editing the Dockerfile. `Cargo.toml`'s `[profile.release]` also sets `codegen-units = 1` — `CARGO_BUILD_JOBS=1` only serializes which crate rustc compiles at a time; a single rustc invocation still parallelizes codegen units (16 by default) across threads otherwise, each with its own LLVM context.
- **russh `Handler` channel-request methods default to silent no-ops**: `pty_request`, `env_request`, `shell_request`, `window_change_request`, `exec_request`, `subsystem_request` all have default trait impls that just `Ok(())` — they never call `session.channel_success(channel)`/`channel_failure(channel)`. If `T2tHandler` doesn't override one, a client's `want_reply` request on that channel (e.g. an interactive, non-`-N` OpenSSH client's `pty-req`/`shell` after opening its session channel) never gets acknowledged, and the client eventually tears down the connection a few seconds later — surfacing server-side only as a bare "connection closed (authenticated)" with no visible cause. `channel_open_session`'s overrides in `lib.rs` now implement all six, acking success for pty/env/shell/window-change and failure for exec/subsystem (no real shell is offered — only chat + tunnel bridging). Also: the SSH accept loop's `session.await` result must be logged on `Err`, not discarded with `let _ =` — that's the only way to see a real russh-level session error if this class of bug recurs.
