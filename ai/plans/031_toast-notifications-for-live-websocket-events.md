...
- **One route, one connection, scope switch via command frame**, not three routes/three connections. `/api/entities/{id}/...` and `/api/admin/...` are both retired in favor of `/api/me/live-connections/ws` (URL kept as-is despite no longer being "mine-only" once scope flips — renaming isn't worth the churn).
...
5. **`crates/tunnel2tunnel-web/src/routes/live_ws.rs`** — replace `run_loop` and all three handler fns with one `my_live_connections_ws` (name kept) holding per-connection mutable scope state:
   ```rust
   pub async fn my_live_connections_ws(AuthUser(user): AuthUser, State(state): State<AppState>, ws: WebSocketUpgrade) -> impl IntoResponse {
...
6. **`frontend/src/stores/liveConnections.ts`** (new Pinia store) — owns the *one* app-wide connection to `/api/me/live-connections/ws`:
   ...
   - `connect()` / `disconnect()` — manual lifecycle, not component-mount-tied. Extract the reconnect/backoff logic out of `composables/useLiveSocket.ts` into a plain `createLiveSocket(path, onMessage, { onSend })` helper returning `{ close(), send(cmd) }`; the store calls it directly (no components open their own socket anymore — `useLiveSocket.ts` itself may become dead code once this store is the only consumer, remove it if so).
