...
5. **Trap — slow auth**: extend `repeated_bad_key_attempts_trigger_slow_auth_delay`: each iteration's `handle` already drops per-iteration; after the loop, query the latest `peer_ip = '127.0.0.1'` row (the 6th, slow-auth-delayed attempt) and apply the shared assertion with `min_gap = Duration::from_secs(3)`. `tarpit::slow_auth::BASE_DELAY` (`crates/tunnel2tunnel-ssh/src/tarpit/slow_auth.rs:8`) is `3s` plus up to `4s` of jitter and isn't reachable from the test crate (`mod tarpit;` in `lib.rs` is private) — duplicate the guaranteed-minimum `3s` with a comment pointing at that const so the two stay in sync. This asserts the row's `ended_at` actually reflects having sat through the slow-auth delay, not just "some later timestamp."

6. **Trap — banner drip** (previously skipped for speed — now included per explicit request): extend `repeated_bans_eventually_engage_banner_drip`. After reading the first drip line, close the raw socket (`drop(socket)` / `socket.shutdown()`) so the server's next `write_all` fails and its drip loop exits; sleep `11s` (one second longer than `banner_drip.rs`'s private `DRIP_INTERVAL = 10s`, `crates/tunnel2tunnel-ssh/src/tarpit/banner_drip.rs:24` — also unreachable from the test crate, duplicate with the same kind of comment); then query the latest row with `tarpit_method = 'banner_drip'` and apply the shared assertion with `min_gap = Duration::from_secs(10)`. This test becomes the slowest in the suite (~11s) — acceptable since the user wants every path covered regardless of runtime.
