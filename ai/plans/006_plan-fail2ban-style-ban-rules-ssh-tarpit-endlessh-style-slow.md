...
- `auth_password`: log the raw attempted password via the new `attempted_password` param; if `tarpit_method == Some(SlowAuth)`, `tarpit::slow_auth::delay().await` before rejecting. If `tarpit_method == Some(FakeShell)`, accept into the fake shell instead of rejecting (same substitution as described for `auth_publickey` below) — password auth is exactly the kind of always-bogus attempt a fake shell should swallow.
- `auth_keyboard_interactive`: currently rejects with no log call at all — **add** a `log_auth_failure` call (`"unsupported auth method"`) to close this counting gap, and apply the same tarpit treatment as `auth_password`: `SlowAuth` → delay before reject; `FakeShell` → accept into the fake shell instead of rejecting.
...
- New `pages/AdminConnectionLogsPage.vue`: filter bar (peer_ip, user, success, method, free-text search) + `data-table` (reuse existing `.data-table` styling) + simple Prev/Next pager using `total`/`page_size`. Each row gets a **"Ban" button** (hidden/disabled for rows that are already covered by an active ban rule) that opens the same add-rule form as `AdminBanRulesPage.vue` pre-filled from that row (`scope_type='peer_ip'` + `peer_ip` from the row, or `scope_type='user'` + `user_id` if the row has one), calling the same `adminApi.createBanRule(...)` — lets an admin ban directly from a log entry instead of round-tripping to the separate rules page.
...
- **e2e, no subprocess/shell-out** (new `crates/t2t/tests/tarpit_e2e.rs`): per user instruction, these new tests must not spawn any external process (`Command::new`/`std::process`) — unlike `tunnel_e2e.rs`'s existing OpenSSH-subprocess harness, which is left untouched and not reused for this file. Instead add a small **in-process `russh::client` test harness** (a minimal `russh::client::Handler` impl accepting any host key, since `russh::client` is not used anywhere in the repo yet but is part of the same `russh` dependency already in `Cargo.toml`) that connects over a plain `tokio::net::TcpStream`/`russh::client::connect_stream`, does real publickey auth, and can open a session channel — giving full control over the exact auth sequence (including deliberately sending only `none`, or offering an unknown key, or doing publickey auth for real) without ever touching a subprocess. Reuses `tunnel_e2e.rs`'s non-process fixtures: DB bootstrap, `generate_test_keypair`, `free_port`, server-start-and-poll.
  - **Required legit-login-sequence test**: pre-seed enough prior failures (or a `ban_rules` row) to put a peer_ip/entity well over threshold, then register a real entity+keypair for that *same* identity and run the exact reference sequence (`ai/errors/4.legitimiate.txt`) via the in-process `russh::client` harness. Assert: login completes (no hang) within a short timeout; resulting `connection_logs` row has `success=true`, `success_reason='correct login'`, `tarpit_method IS NULL`.
  - `auth_none`-only probing (connect+probe+disconnect, repeated, via the same client harness stopping right after the initial exchange) never crosses the threshold and never writes a `connection_logs` row.
  - Banner-drip: raw `TcpStream` (never even constructs a `russh::client` — the whole point is the real ID line never arrives) connects after threshold crossed with round-robin landing on `BannerDrip`; asserts repeated non-`"SSH-"` lines arrive and the real ID string never does within a bounded wait.
  - Slow-auth / fake-shell: drive via the `russh::client` harness with a genuinely unknown key (so real auth legitimately fails) once tarpit method is forced/seeded to each value; assert elapsed time is higher for slow-auth, and for fake-shell assert the harness's auth call reports success (server returned `Auth::Accept`) and a subsequently opened channel receives the bogus prompt bytes, then the test explicitly closes the connection (no hang risk since it's all in-process async, not a real client waiting on a real terminal).

## 7. Online/offline badge (entity last-seen, piggybacks on connection_logs)

Reuses the already-tracked `connection_logs.ended_at` (set via `ConnectionLog::set_ended` in `impl Drop for T2tHandler`) — no new column needed: an entity is "online" iff it has a `connection_logs` row with `success = true AND ended_at IS NULL`; its "last disconnected at" is `MAX(ended_at)` across its successful rows.

- `tunnel2tunnel-core`: new `ConnectionLog::entity_status(pool, entity_id) -> (bool /* online */, Option<OffsetDateTime> /* last_disconnected_at */)`, plus a batch variant `ConnectionLog::entity_statuses(pool, entity_ids: &[Uuid]) -> HashMap<Uuid, (bool, Option<OffsetDateTime>)>` so the entities-list page doesn't do N+1 queries (single `SELECT entity_id, bool_or(ended_at IS NULL) AS online, MAX(ended_at) AS last_disconnected_at FROM connection_logs WHERE entity_id = ANY($1) AND success GROUP BY entity_id`).
- `tunnel2tunnel-web`: `EntityResponse` (routes/entities.rs) gains `online: bool` and `last_disconnected_at: Option<String>` (RFC3339), populated via the batch query in both the list and single-entity handlers.
- Frontend: `api/entities.ts` `Entity` interface gains `online`/`last_disconnected_at`; a small badge (green "Online" / grey "Offline — last seen {relative time}") shown next to the entity name in `EntitiesPage.vue`'s list and `EntityDetailPage.vue`'s header, following existing badge styling (`.badge-result` class used for the connection-log success/fail badge is a reasonable precedent to reuse/extend).
- Test: `connection_log.rs` unit test (`#[sqlx::test]`) — entity with an open (`ended_at IS NULL`) successful session reads `online=true`; after `set_ended`, `online=false` and `last_disconnected_at` matches.
...
7. New tarpit e2e tests use an in-process `russh::client` harness instead of shelling out to a real `ssh` binary (per explicit user instruction to avoid sys/cmd calls in tests) — `tunnel_e2e.rs`'s existing OpenSSH-subprocess approach is left as-is and not touched/reused.
